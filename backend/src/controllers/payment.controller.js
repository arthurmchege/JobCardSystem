...
const initiatePayment = async (req, res) => {
  try {
    ...
    // Check if Job exists
    const jobResult = await db.query("SELECT * FROM job_cards WHERE id = $1", [
      job_card_id,
    ]);
    ...
    await db.query("UPDATE job_cards SET payment_status = $1 WHERE id = $2", [
      "pending",
      job_card_id,
    ]);
    ...
    if (!stkResult.success) {
      await db.query("UPDATE payments SET payment_status = $1 WHERE id = $2", [
        "failed",
        payment.id,
      ]);
      await db.query("UPDATE job_cards SET payment_status = $1 WHERE id = $2", [
...
const handleCallback = async (req, res) => {
  try {
    ...
    if (resultCode === 0) {
      ...
      const transactionDateRaw = metadata.find(
      ...
      await logEvent("payment_success", null, {
...
const getJobPayments = async (req, res) => {
  try {
    ...
    // Check if job exists
    const jobResult = await db.query("SELECT id FROM job_cards WHERE id = $1", [
      jobId,
    ]);
