...
const getAllJobCards = async (filters = {}) => {
    ...
    if (status) {
        if (Array.isArray(status)) {
            ...
            const placeholders = status.map((_, index) => `$${paramCounter + index}`).join(', ');
    ...
    if (start_date && end_date) {
        conditions.push(`jc.scheduled_date BETWEEN $${paramCounter} AND $${paramCounter + 1}`);
    ...
    if (search) {
        ...
        conditions.push(`(jc.title ILIKE $${paramCounter} OR jc.description ILIKE $${paramCounter})`);
        params.push(`%${search}%`);
        paramCounter++;
    }
...
const createJobCard = async (jobCardData) => {
    ...
    if (technicianCheck.rows[0].role !== 'technician') {
        const error = new Error(`User with ID ${technician_id} is not a technician (role: ${technicianCheck.rows[0].role})`);
        error.statusCode = 400;
        throw error;
    }
...
const completeJobCard = async (jobCardId, completionData) => {
    ...
    if (currentJob.status === 'pending') {
        const error = new Error('Job must be started before it can be completed. Please start the job first.');
        error.statusCode = 400;
        throw error;
    }
